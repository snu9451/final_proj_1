...
//		return mav;
	...
	public void selectCoin(HttpServletRequest req, HttpServletResponse res) {
		...
		pmap.put("mem_email", mem_email);
		hmb.bindPost(pmap);
		pmap = memberLogic.selectMemberAdmin(pmap);
		int coin = Integer.parseInt((pmap.get("COIN_REMAIN")).toString());
		
		// 세션 갱신하기
		login.put("COIN_REMAIN", coin);
		session.setAttribute("login", login);
		
		
		logger.info("************남은 코인 얼마냐*********** : "+coin);
	    //login.put("COIN_REMAIN",coin);
	    //session.setAttribute("login", login);
	    logger.info("**********정보들*********"+login);
	}
	...
	public void updateNickName(HttpServletRequest req, HttpServletResponse res) {	// ♣ 완료
		...
		if(result == 1) {
			...
			// 세션 갱신하기
			login.put("MEM_NICKNAME", mem_nickname);
			session.setAttribute("login", login);
		}
