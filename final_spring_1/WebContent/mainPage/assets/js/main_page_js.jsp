...
<script type="text/javascript">
  ...
  $(document).ready(function () {
    ...
    function chckSearch__rank(product_top, scroll_top) {
    ...
    chckSearch__rank(product_top, scroll_top);


	  //스크롤 이동시 이벤트
	  $(window).scroll(function () {
	    //중고상품 섹션 top 위치값
	    product_top = product.getBoundingClientRect().top;
	    //현재 스크롤 위치값
  ...
  function itemSearch() {
    ...
    $.ajax({
      ...
      //     	data: search,
      url:
        "http://localhost:9696/item/selectBySearch.nds?pr_search=" +
  ...
  function viewItemList(items) {
    ...
    if (items.length > 0) {
      ...
      itemAll.innerHTML =
        "<div style='display: block; width: 100%; text-align: center;'>조회결과가 없습니다<div>";
    }
  }
  ...
  function pdCommentBtn(pdComment) {
    ...
    pdCommentform +=
      "					</form>                                                                                                                   ";
    pdComment.parentNode.parentNode.parentNode.parentNode.parentNode.innerHTML +=
      pdCommentform;
  }
  //댓글 수정
  function pdCommentupdateBtn(pdComment) {
    let history =
      pdComment.parentNode.parentNode.parentNode.parentNode.nextElementSibling
        .innerText;
    let pdCommentform = "";
    pdCommentform += "					<form action=''>";
    pdCommentform +=
      "							<textarea class='w-100' row='2'>" + history + "</textarea> ";
    pdCommentform +=
      "							<div class='input-group-prepend'>                                                                                 ";
    pdCommentform +=
      "								<button type='button' class='btn btn-primary' onclick='insertComment(this)'>댓글수정</button>";
    pdCommentform += "							</div> ";
    pdCommentform += "					</form>  ";
    pdComment.parentNode.parentNode.parentNode.parentNode.nextElementSibling.innerHTML =
      pdCommentform;
  }
