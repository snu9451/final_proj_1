...
<script type="text/javascript">
		...
		$.ajax({
			url: 'http://localhost:4444/member/reqLoginView.nds?mem_sessionid='+mem_sessionid,
			type: 'get',
			dataType: 'json',
...
// 		$('#logIn').modal('hide');
		swal("가입하신 이메일을 입력해주세요.", {
			...
			.then((value) => {
				...
				$.ajax({
					url: 'http://localhost:4444/member/issueTempPw.nds',
					data: dt,
					type: 'post',
					dataType: 'html',
					success: function(data){
...
//						location.reload();						
				...
				swal("환영합니다. "+data[1]+"님!")
					.then(function(value){
						...
						location.href = "http://localhost:4444/mainPage/main_page.nds"
					});
//					/* ============== [[ 공통코드: 폼 비워주기 ]] ============== */
//					$('#f_login input').each(function(index){
//						$('#f_login input').eq(index).val('');
