...
$(document).ready(function(){
	...
	$('#h_itemInsert').on('click', function(){
		location.href = "http://localhost:8080/item/itemUploadPage.nds";
	});
	...
	$('#category_drop').on('click', function(){		
	    ...
	    if($("#category_item").children().length < 1){
		    $.ajax({
		      url: "http://localhost:8080/etc/selectCategory.nds",
...
// 		        		alert($('.dropdown-item').eq(index).text());
		        		...
		        		location.href = "http://localhost:8080/item/selectByCategory.nds?pr_categori="+cate;
		        	});
		          });
		      },
		    	error:function(e){
