...
import com.example.android_nds.databinding.FragmentAllListItemBinding
...
class AllListItemFragment : Fragment() {

    //메인 UI
    private lateinit var  allListItembinding: FragmentAllListItemBinding
    val binding get()= allListItembinding!!
    //어댑터 아이템 리스트
...
//        }
        ...
        val okHttpClient = OkHttpClient.Builder()
            .addInterceptor(activity?.applicationContext?.let { LoginHandleInterceptor(it) }).build()
...
//                binding.recyclerviewWrapper.scrollTo(0,0)
...
//                }
    ...
    private fun initSearchEditText(){
        ...
        //화면에서 ACTION_DOWN은 [버튼을 눌렀을 때] , ACTION_UP은 [손을 땠을때]를 의미한다.
        //따라서, 엔터를 손가락으로 누르고 있을때 이벤트가 발생할 것임을 말해준다.
        binding.searchEditText.setOnKeyListener{v,keyCode, event ->
            if(keyCode == KeyEvent.KEYCODE_ENTER && event.action == MotionEvent.ACTION_DOWN){
                Log.i(TAG, "에디트 텍스트 이벤트 발생!")
            ...
            .enqueue(object: Callback<List<Items>> {
                ...
                override fun onResponse(call: Call<List<Items>>, response: Response<List<Items>>) {
                    if(response.isSuccessful.not()){
                        Log.e("main_fail","response.isSuccessful.not() ==> Not Success!!")
                        return
                    }
                    //검색어들 안보이게
                    hideHistoryView()
                ...
                override fun onFailure(call: Call<List<Items>>, t: Throwable) {
                    Log.e("main_fail", "onFailure"+t.toString())
                }
...
//            R.id.categori ->
//                activity?.supportFragmentManager?.beginTransaction()
//                    ?.replace(R.id.allListIemFrag, ItemCategoriFragment())
//                    ?.commit()
//        }
